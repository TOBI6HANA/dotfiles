...
local enter = function(hovered_url, is_dir)
	if hovered_url and is_dir then
		-- Route ordinary directories through augment-command so its
		-- single-child-directory skipping is retained.
		ya.emit("plugin", { "augment-command.enter" })
	else
	...
	entry = function(_, job)
		...
		elseif action == "leave" then
			if not is_mount_point() then
				-- Keep augment-command's matching skip behavior when leaving
				-- normal directories; mounted archives use the branch below.
				ya.emit("plugin", { "augment-command.leave" })
				return
			end
